...
则 $div\ (f(x(t),t)) = div\ (-\frac{1}{2}\sigma^{2t}s_\theta(x,t)) = -\frac{1}{2} \dfrac{d\sigma^{2t}}{dt} div\ (s_\theta(x,t))$，代入上式，并将两边同时取对数可得：</p>
...
<center>$$x(t+\triangle t) \leftarrow x(t+\triangle t) + \epsilon \triangledown_xlog\ p_{\theta,(t +\triangle t)}(x) + \sqrt{2\epsilon} z, z \sim \mathcal{N}(0,I) \\ x(t+\triangle t) \leftarrow x(t+\triangle t) + \epsilon s_\theta(x, t+\triangle t) + \sqrt{2\epsilon} z$$</center>