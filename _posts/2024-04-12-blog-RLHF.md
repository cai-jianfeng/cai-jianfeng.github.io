...
<p style="text-align: justify; text-justify: inter-ideograph; word-break: break-all;">可以看到，$\hat{A}_t$ 的计算是从后往前推的，它需要先计算出整个 trajectory；因此，$\hat{A}_t$ 的计算是基于上一次迭代生成的 trajectory：</p>

$$\begin{aligned}
\text{上一次迭代的 trajectory 为：} & \\
& \{(s_0, a_0, V(s_0)), \\
& \quad (s_1, a_1, V(s_1)), \\
& \quad \vdots \\
& \quad (s_m, a_m, V(s_m))\} \\
\text{然后通过上述计算公式得到：} & \\
& \hat{A} = \{\hat{A}_m, ..., \hat{A}_0\} \\
\text{最后将 $\hat{A}$ 输入到下一次迭代进行优化函数值的计算。} &
\end{aligned}$$
