...
每个 expert 模型的架构基本和 FFN 一致，因此后续工作基本集中在<b> Gating 模型的设计</b>和<b>总体模型的训练</b>上。</p>
...
$$\mathbf{v}=\operatorname{relu}\left(\left[A_{k_1}(\mathbf{I^l}), \ldots, A_{k_N}(\mathbf{I^l})\right]\right) \quad \operatorname{FFN}(\mathbf{I^l})=\left[B_{k_1}(\mathbf{v}), \ldots, B_{k_N}(\mathbf{v})\right] \\
O^l = FFN_{hash(x)}(I^l)$$</p>
