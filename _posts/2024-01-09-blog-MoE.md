...
1. <p style="text-align:justify; text-justify:inter-ideograph;"><a href="https://proceedings.neurips.cc/paper_files/paper/2021/hash/92bf5e6240737e0326ea59846a83e076-Abstract.html" target="_blank" title="Hash Layers For Large Sparse Models">Hash</a>：该方法使用 hash 的方法来实现 Gating Model。
...
1. <p style="text-align:justify; text-justify:inter-ideograph;"><a href="https://arxiv.org/abs/1701.06538" target="_blank" title="Outrageously Large Neural Networks: The Sparsely-Gated Mixture-of-Experts Layer">Sparsely-Gated</a>：
...
1. <p style="text-align:justify; text-justify:inter-ideograph;"><a href="https://dl.acm.org/doi/abs/10.5555/3586589.3586709" target="_blank" title="Switch transformers: scaling to trillion parameter models with simple and efficient sparsity">Switch Transformers</a>：
...
$$O = \sum_{i=1}^KG(x)_iH_i(x); G(x) = softmax(KeepTop1(x \times W_g, k)) \\
\text { KeepTop1 }(v, k)_i= \begin{cases}v_i & \text { if } v_i = max(v) \\ -\infty & \text { otherwise. }\end{cases}$$<span style="color: red">(补充 Load Balancing loss + 改进的 Training and Fine-Tuning 方法)</span></p>
...
1. <p style="text-align:justify; text-justify:inter-ideograph;"><a href="https://proceedings.mlr.press/v162/du22c.html" target="_blank" title="GLaM: Efficient Scaling of Language Models with Mixture-of-Experts">GLaM</a>：
...
$$O = \sum_{i=1}^KG(x)_iH_i(x); G(x) = softmax(KeepTop2(x \times W_g, k)) \\
\text { KeepTop2 }(v, k)_i= \begin{cases}v_i & \text { if } v_i \text { is in the top } 2 \text { elements of } v \\ -\infty & \text { otherwise. }\end{cases}$$<span style="color: red">(补充 Training Dataset + 改进的 Transformer FFN)</span></p>

![GLaM](/images/paper_GLaM.png)

1. <p style="text-align:justify; text-justify:inter-ideograph;"><a href="https://arxiv.org/abs/2006.16668" target="_blank" title="GShard: Scaling Giant Models with Conditional Computation and Automatic Sharding">GShard</a>：</p>
