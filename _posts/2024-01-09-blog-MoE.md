...
$$O = H_{hash(x)}(x)$$<span style="color: red">(补充其在 Transformer 框架的具体应用以及 Multihash-Layer)</span></p>
...
\text { KeepTopK }(v, k)_i= \begin{cases}v_i & \text { if } v_i \text { is in the top } k \text { elements of } v \\ -\infty & \text { otherwise. }\end{cases}$$</p>
