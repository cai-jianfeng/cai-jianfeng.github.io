...
为了尽可能保证各个 expert 模型负载平衡，它使用 Local Group Dispatching：将 $1$ 个 batch $\mahtcal{B}$ 的数据(数据量为 $N$)平均分成 $G$ 份(则每份为 $S = \dfrac{N}{G}$)。
...
c_e \text{是第 e 个 expert 模型所分配到的数据量}; m_e = \dfrac{1}{S} g_{s,e} \text{是 Gating 模型分配给所有数据针对第 e 个 expert 模型的权重之和}$$
