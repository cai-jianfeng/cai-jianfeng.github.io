...
$$\begin{align}P(x, i) & =\operatorname{Pr}\left(\left(x \cdot W_g\right)_i+\text { StandardNormal }() \cdot \operatorname{Softplus}\left(\left(x \cdot W_{\text {noise }}\right)_i\right) > \operatorname{kth_excluding}(H(x), k, i)\right) \\
...
其中，$\text { kth_excluding }(v;k;i)$ 表示 $v$ 的第 $k$ 个最高分量，不包括分量 $i$；$\Phi$ 表示标准正态分布的累积分布函数。
...
<span style="color: red">(补充 shrinking batch + network bandwidth)</span></p>
