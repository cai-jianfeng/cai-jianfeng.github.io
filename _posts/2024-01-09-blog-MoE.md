...
- <p style="text-align:justify; text-justify:inter-ideograph;">名词解释：</p>
  ...
  - <p style="text-align:justify; text-justify:inter-ideograph;">Gated Linear Unit：</p>
...
而对于负载损失，V-MoE 将 $\operatorname{kth\_excluding}(v;k;i)$ 函数简化为阈值：
...
\mathcal{L}(X)=\mathcal{L}_{\text{classification}}(X)+\lambda \mathcal{L}_{\text{aux}}(X); \lambda = 0.01$$
<span style="color: red">(补充 load balancing losses)</span></p>

![V-MoE](/images/paper_V-MoE.png)

![V-MoE routing algorithm](/images/paper_V)
