...
- <p style="text-align:justify; text-justify:inter-ideograph;">名词解释：</p>
  - <p style="text-align:justify; text-justify:inter-ideograph;">expert capacity：</p>
...
\text { KeepTopK }(v, k)_i= \begin{cases}v_i & \text { if } v_i \text { is in the top } k \text { elements of } v \\ -\infty & \text { otherwise. }\end{cases}$$
为了减轻负载不均衡问题，引入<b>负载均衡损失</b>函数，其将 Sparsely-Gated 中的<b>重要性损失</b>和<b>负载损失函数</b>的结合与简化。
...
其中，$\alpha$ 为乘积因子；而 $\time N$ 是为了使损失与 expert 模型的数量无关
(因为 $\sum_{i=1}^N(f_i \cdot P_i) \propto \sum_{i=1}^N(\dfrac{1}{N} \cdot \dfrac{1}{N} = \dfrac{1}{N})$)。
<span style="color: red">(补充改进的 Training and Fine-Tuning 方法)</span></p>
